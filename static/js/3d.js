...
async function renderCollabSection() {
  ...
  const filtered = rows
    .filter(row => String(row.group || "").trim().toLowerCase() === "3d")
...
async function renderPortfolioSection() {
  ...
  if (!items3D.length) return;

  const eyeItems = items3D.filter(
    row => String(row.category || "").trim() === "눈알"
  );

  const headItems = items3D.filter(
    row => String(row.category || "").trim() === "헤드"
  );

  const threeViewItems = items3D.filter(
    ...
    <div class="pf-wrap">
      ...
      <div class="pf-stack">
        ...
        ${createPortfolioLoopSection({
          ...
          items: headItems,
          columns: 2,
          sectionClass: "is-head"
        })}

        ${createPortfolioLoopSection({
          title: "삼면도",
          items: threeViewItems,
          columns: 1,
