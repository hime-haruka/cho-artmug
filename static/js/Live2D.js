...
async function renderIntroSection() {
  ...
  const [profileRows, slotRows] = await Promise.all([
    fetchCSV(SHEET_URLS.introProfile),
    fetchCSV(SHEET_URLS.introSlots)
  ]);

  const profile = profileRows[0];
  if (!profile && !slotRows.length) return;
  ...
  if (slotRows.length) {
    body.appendChild(createIntroSlotsCard(slotRows));
  }
...
async function renderCollabSection() {
  ...
  const filtered = rows
    .filter(row => String(row.group || "").trim().toLowerCase() === "live2d")
...
/* =========================
   portfolio urls
========================= */
const PORTFOLIO_INFO_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vTDYdEQ_sxNX4u2b_n1kHDayISAVa2KumYZ-MqmkxPJncDyJBuf07UuKxxxO3Gi0_PWv2Fkc88wxGcK/pub?gid=842573584&single=true&output=csv";
...
function createPortfolioInfoBlock(type, items = []) {
  const listHtml = items
    .sort((a, b) => Number(a.order || 0) - Number(b.order || 0))
    .map((item, index) => {
      return `
        <li class="portfolio-info__item">
          <span class="portfolio-info__num">${index + 1}.</span>
          <span class="portfolio-info__text">
            <strong>${escapeHtml(item.title || "")}</strong>
            <span class="portfolio-info__desc">${escapeHtml(item.desc || "")}</span>
          </span>
        </li>
      `;
    })
  ...
  return `
    <section class="portfolio-info">
      <div class="portfolio-info__head">
        <p class="portfolio-info__eyebrow">PORTFOLIO TYPE</p>
        <h3 class="portfolio-info__title">${escapeHtml(type)}</h3>
      </div>
...
function createShotCard(item = {}) {
  const imageUrl = normalizeImageUrl(item.image_url || "");
  const name = item.name || "";
  const desc = item.desc || "";
  const altText = escapeHtml(name || item.sub || item.category || "포트폴리오 이미지");

  return `
    <figure class="portfolio-shot">
      <a
        class="portfolio-shot__link"
        href="${imageUrl || "#"}"
        ${imageUrl ? 'target="_blank" rel="noopener noreferrer"' : ""}
        aria-label="${altText} 크게 보기"
      >
        <div class="portfolio-shot__media">
          ${
            imageUrl
              ? `<img src="${imageUrl}" alt="${altText}" loading="lazy">`
              : `<div class="portfolio-shot__placeholder">이미지 준비중</div>`
          }
        </div>
      </a>
        ...
        name || desc
          ? `
            <figcaption class="portfolio-shot__caption">
              ${name ? `<strong class="portfolio-shot__name">${escapeHtml(name)}</strong>` : ""}
              ${desc ? `<span class="portfolio-shot__desc">${escapeHtml(desc)}</span>` : ""}
            </figcaption>
          `
          : ""
      }
...
function createSliderSection({ title, items = [], columns = 2, sectionClass = "" }) {
  if (!items.length) {
    return `
      <section class="portfolio-block ${sectionClass}">
  ...
  const pagesHtml = pages
    .map((pageItems, index) => {
      return `
        <div class="portfolio-slider__page" data-page="${index}">
          <div class="portfolio-grid portfolio-grid--${columns}">
            ${pageItems.map(createShotCard).join("")}
          </div>
        </div>
      `;
    })
  ...
  return `
    <section class="portfolio-block ${sectionClass}">
      <div class="portfolio-block__head">
        <h3 class="portfolio-block__title">${escapeHtml(title)}</h3>
      </div>

      <div class="portfolio-slider" data-columns="${columns}">
        <button type="button" class="portfolio-slider__nav is-prev" aria-label="${escapeHtml(title)} 이전">
          <span></span>
        </button>
        ...
        <button type="button" class="portfolio-slider__nav is-next" aria-label="${escapeHtml(title)} 다음">
          <span></span>
        </button>
      </div>
    </section>
...
function createRigSubSection(subTitle, items = []) {
  return `
    <section class="portfolio-rig">
      <div class="portfolio-rig__head">
        <h4 class="portfolio-rig__title">${escapeHtml(subTitle)}</h4>
      </div>

      <div class="portfolio-rig__grid">
        ${items
          .sort((a, b) => Number(a.order || 0) - Number(b.order || 0))
          .map(createShotCard)
          .join("")}
...
function createPortfolioLayout(infoRows = [], galleryRows = []) {
  const live2dGallery = galleryRows.filter(
    row => String(row.group || "").trim().toLowerCase() === "live2d"
  );

  const illustrationInfo = infoRows.filter(
    row => String(row.type || "").trim() === "일러스트"
  );

  const rigInfo = infoRows.filter(
    row => String(row.type || "").trim() === "일러스트 + 리깅"
  );

  const illustrationSamples = live2dGallery.filter(
    row => String(row.category || "").trim() === "일러스트"
  );

  const threeViewSamples = live2dGallery.filter(
    row => String(row.category || "").trim() === "삼면도"
  );

  const rigSamples = live2dGallery.filter(
    row => String(row.category || "").trim() === "리깅"
  );

  const rigGroups = groupBy(
    rigSamples.sort((a, b) => Number(a.order || 0) - Number(b.order || 0)),
    row => String(row.sub || "").trim() || "기타"
  );

  const rigOrder = ["눈 깜빡임", "얼굴 움직임", "몸 움직임", "기타"];
  const orderedRigKeys = [
    ...rigOrder.filter(key => rigGroups[key]),
    ...Object.keys(rigGroups).filter(key => !rigOrder.includes(key))
  ];
  ...
  return `
    <div class="portfolio-wrap">
      ...
      <div class="portfolio-stack">
        ${createPortfolioInfoBlock("일러스트", illustrationInfo)}

        ${createSliderSection({
          title: "일러스트",
          items: illustrationSamples.sort((a, b) => Number(a.order || 0) - Number(b.order || 0)),
          columns: 2,
          sectionClass: "is-illust"
        })}

        ${createSliderSection({
          title: "삼면도",
          items: threeViewSamples.sort((a, b) => Number(a.order || 0) - Number(b.order || 0)),
          columns: 1,
          sectionClass: "is-threeview"
        })}

        ${createPortfolioInfoBlock("일러스트 + 리깅", rigInfo)}

        <section class="portfolio-block portfolio-block--rig">
          <div class="portfolio-rig-list">
            ${orderedRigKeys.map(key => createRigSubSection(key, rigGroups[key])).join("")}
          </div>
        </section>
      </div>
    </div>
...
function initPortfolioSliders(root = document) {
  const sliders = root.querySelectorAll(".portfolio-slider");

  sliders.forEach(slider => {
    const viewport = slider.querySelector(".portfolio-slider__viewport");
    const track = slider.querySelector(".portfolio-slider__track");
    const pages = [...slider.querySelectorAll(".portfolio-slider__page")];
    const prevBtn = slider.querySelector(".portfolio-slider__nav.is-prev");
    const nextBtn = slider.querySelector(".portfolio-slider__nav.is-next");

    if (!viewport || !track || !pages.length || !prevBtn || !nextBtn) return;
    ...
    const update = (animate = true) => {
      ...
      track.style.transform = `translate3d(-${pageWidth * current}px, 0, 0)`;
...
async function renderPortfolioSection() {
  ...
  const [infoRows, galleryRows] = await Promise.all([
    fetchCSV(PORTFOLIO_INFO_URL),
    fetchCSV(PORTFOLIO_GALLERY_URL)
  ]);

  if (!infoRows.length && !galleryRows.length) return;

  section.innerHTML = createPortfolioLayout(infoRows, galleryRows);
  initPortfolioSliders(section);
}
